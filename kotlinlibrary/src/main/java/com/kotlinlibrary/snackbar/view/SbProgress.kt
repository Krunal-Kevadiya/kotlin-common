...
class SbProgress : View {
    ...
    constructor(context: Context, attrs: AttributeSet) : super(context, attrs) {
        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.SbProgress))
        setAnimationEnabled()
    }
    ...
    private fun parseAttributes(a: TypedArray) {
        ...
        circleRadius = a.getDimension(R.styleable.SbProgress_sbp_circleRadius,
                circleRadius.toFloat()).toInt()

        fillRadius = a.getBoolean(R.styleable.SbProgress_sbp_fillRadius, false)

        barWidth = a.getDimension(R.styleable.SbProgress_sbp_barWidth, barWidth.toFloat()).toInt()

        rimWidth = a.getDimension(R.styleable.SbProgress_sbp_rimWidth, rimWidth.toFloat()).toInt()

        val baseSpinSpeed = a.getFloat(R.styleable.SbProgress_sbp_spinSpeed, spinSpeed / 360.0f)
        spinSpeed = baseSpinSpeed * 360

        barSpinCycleTime = a.getInt(R.styleable.SbProgress_sbp_barSpinCycleTime,
                barSpinCycleTime.toInt()).toDouble()

        barColor = a.getColor(R.styleable.SbProgress_sbp_barColor, barColor)

        rimColor = a.getColor(R.styleable.SbProgress_sbp_rimColor, rimColor)

        linearProgress = a.getBoolean(R.styleable.SbProgress_sbp_linearProgress, false)

        if (a.getBoolean(R.styleable.SbProgress_sbp_progressIndeterminate, false)) {
            spin()
        }
    ...
    internal class WheelSavedState : View.BaseSavedState {
        ...
        private constructor(parcel: Parcel) : super(parcel) {
            ...
            this.isSpinning = parcel.readByte().toInt() != 0
            ...
            this.linearProgress = parcel.readByte().toInt() != 0
            this.fillRadius = parcel.readByte().toInt() != 0
        }
