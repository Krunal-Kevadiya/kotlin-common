...
fun <R: Any?, E: Any?> Response<R>.toSealedApiResult(responseType: Type, errorType: Type): Some<R, E> {
    return when (code) {
        ...
        HttpStatusCode.SWITCHING_PROTOCOLS -> SwitchingProtocols101(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PROCESSING -> Processing102(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.EARLY_HINTS -> EarlyHints103(headers = headers, errorBody = errorBody(responseType))
        ...
        HttpStatusCode.MULTIPLE_CHOICES -> MultipleChoices300(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.MOVED_PERMANENTLY -> MovedPermanently301(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.FOUND -> Found302(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.SEE_OTHER -> SeeOther303(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NOT_MODIFIED -> NotModified304(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.USE_PROXY -> UseProxy305(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.TEMPORARY_REDIRECT -> TemporaryRedirect307(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PERMANENT_REDIRECT -> PermanentRedirect308(headers = headers, errorBody = errorBody(responseType))

        HttpStatusCode.BAD_REQUEST -> BadRequest400(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.UNAUTHORIZED -> Unauthorized401(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PAYMENT_REQUIRED -> PaymentRequired402(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.FORBIDDEN -> Forbidden403(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NOT_FOUND -> NotFound404(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.METHOD_NOT_ALLOWED -> MethodNotAllowed405(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NOT_ACCEPTABLE -> NotAcceptable406(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PROXY_AUTHENTICATION_REQUIRED -> ProxyAuthenticationRequired407(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.REQUEST_TIMEOUT -> RequestTimeout408(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.CONFLICT -> Conflict409(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.GONE -> Gone410(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.LENGTH_REQUIRED -> LengthRequired411(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PRECONDITION_FAILED -> PreconditionFailed412(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PAYLOAD_TOO_LARGE -> PayloadTooLarge413(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.URI_TOO_LONG -> URITooLong414(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.UNSUPPORTED_MEDIA_TYPE -> UnsupportedMediaType415(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.RANGE_NOT_SATISFIABLE -> RangeNotSatisfiable416(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.EXPECTATION_FAILED -> ExpectationFailed417(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.IM_A_TEAPOT -> ImATeapot418(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.ENHANCE_YOUR_CALM -> EnhanceYourCalm420(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.MISDIRECTED_REQUEST -> MisdirectedRequest421(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.UN_PROCESSABLE_ENTRY -> UnprocessableEntry422(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.LOCKED -> Locked423(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.FAILED_DEPENDENCY -> FailedDependency424(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.TOO_EARLY -> TooEarly425(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.UPGRADE_REQUIRED -> UpgradeRequired426(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.PRECONDITION_REQUIRED -> PreconditionRequired428(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.TOO_MANY_REQUESTS -> TooManyRequests429(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.REQUEST_HEADER_FIELDS_TOO_LARGE -> RequestHeaderFieldsTooLarge431(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NO_RESPONSE -> NoResponse444(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.RETRY_WITH -> RetryWith449(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.BLOCKED_BY_WINDOWS_PARENTAL_CONTROLS -> BlockedByWindowsParentalControls450(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.UNAVAILABLE_FOR_LEGAL_REASONS -> UnavailableForLegalReasons451(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.CLIENT_CLOSED_REQUEST -> ClientClosedRequest499(headers = headers, errorBody = errorBody(responseType))

        HttpStatusCode.INTERNAL_SERVER_ERROR -> InternalServerError500(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NOT_IMPLEMENTED_ERROR -> NotImplementedError501(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.BAD_GATEWAY -> BadGateway502(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.SERVICE_UNAVAILABLE -> ServiceUnavailable503(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.GATEWAY_TIMEOUT -> GatewayTimeout504(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.HTTP_VERSION_NOT_SUPPORTED -> HTTPVersionNotSupported505(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.VARIANT_ALSO_NEGOTIATES -> VariantAlsoNegotiates506(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.INSUFFICIENT_STORAGE -> InsufficientStorage507(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.LOOP_DETECTED -> LoopDetected508(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.BANDWIDTH_LIMIT_EXCEEDED -> BandwidthLimitExceeded509(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NOT_EXTENDED -> NotExtended510(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NETWORK_AUTHENTICATION_REQUIRED -> NetworkAuthenticationRequired511(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NETWORK_READ_TIMEOUT_ERROR -> NetworkReadTimeoutError598(headers = headers, errorBody = errorBody(responseType))
        HttpStatusCode.NETWORK_CONNECT_TIMEOUT_ERROR -> NetworkConnectTimeoutError599(headers = headers, errorBody = errorBody(responseType))
