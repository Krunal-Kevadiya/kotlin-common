...
open class SingleBindingAdapter<T> (
    ...
    override operator fun plus(itemList: MutableList<T>): DataBindingBaseAdapter<T, DataBindingBaseViewHolder<T>> {
        if(configuration.isDiffUtils) {
    ...
    override fun insert(index: Int, itemList: MutableList<T>): DataBindingBaseAdapter<T, DataBindingBaseViewHolder<T>> {
        if(configuration.isDiffUtils) {
